<template>
  <div :class="loaderClassName">
    <div v-if="type=='loaderNumber'" class="loader-inner"></div>
    <div class="loading pl-5" v-if="type=='normalText'"><span>L</span><span>o</span><span>a</span><span>d</span><span>i</span><span>n</span><span>g</span>
    </div>
  </div>
</template>
<script>
  export default {
    ...
    computed: {
      loaderClassName() {
        let vm = this;
        if(vm.type == 'normalText') return `loader-normal-txt`;
        else if(vm.type == 'loaderNumber') return `loader loader-${vm.typeNumber}`;
