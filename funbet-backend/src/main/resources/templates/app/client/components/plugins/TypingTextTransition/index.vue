<template>
  <div style="position: absolute;top: -1.2em;left: 5em;width: 100%;">
    <div style="position: relative">
    <p id="offscreen-text" class="offscreen-text"></p>
    <p id="text" class="text"></p>

      <svg id="svg"></svg>
      <input type="text" class="input" id="input" v-show="false"/>
    </div>
  </div>
</template>
<script>

  import SVGElement from 'components/plugins/TypingTextTransition/SVGElement.js';
  import {CSSPlugin, EasePack, TweenLite} from "gsap";
  import {Power2, Power3} from "gsap/EasePack";
  import $ from 'jquery';
  export default {
    props: ['startText'],
    computed: {
      prompt(){
          return this.startText.split('')
      }
    },
    mounted() {
      let vm = this;
...
//            console.log(letter);
      ...
      const onInputChange = () => {
        ...
        if(vm.prompt.length == value.length){
          vm.$nextTick(() => {
            vm.$emit('on-rendered-start-text', 'abc');
          }, 400)
        }
      };
      ...
      const addPrompt = (i) => {
        setTimeout(() => {
          if (runPrompt && vm.startText[i]) {
            input.value = input.value + vm.startText[i];
            onInputChange();
            addPrompt(i + 1);
          }
        }, 300);
