...
const withWCPortal = (
  Component,
  { tagName, defaultProps, connectedCallback, disconnectedCallback = () => {} },
) => {
  const WCPortal = () => {
    const ref = useRef();
    ...
    useEffect(() => {
      ...
      if (wcNode) {
        ...
        /**
         * The purpose of these code below to dispatch an event to Legacy only one time,
         * to inform that this component has already assiged a referrence
         */
        if (wcNode._ref && !wcNode.assigedRef) {
          wcNode.assigedRef = true;
          appBootrapNode?.dispatchEvent(
            new window.CustomEvent(`${tagName}-did-assign-ref`),
          );
        }
      }
    });
